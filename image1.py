...
from sql_handler import DBconnection

csv_file = CSVFile("admin_data.csv")
list_data = csv_file.get_data()
database = DBconnection(list_data[0], list_data[1], list_data[2], list_data[3])
database.connect_sql()
sometext = database.run_sql("""SELECT company_name FROM project WHERE id = 1;""")
somenumber = database.run_sql("""SELECT budget_value FROM project WHERE id = 1;""")
